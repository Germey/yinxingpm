  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function createFCBK() {
        ...
        complete = $('<div class="facebook-auto">').width((options.width) * 0.8);
      ...
      function bindEvents() {
        ...
        //Runmiao & Raymond
        //Add focusout event for autocomplete
        maininput.unbind("focusout");
        maininput.focusout( function(event) {
            if (options.newel) {
      ...
      function load_feed(etext){
        counter = 0;
        if ((options.json_url && maxItems()) || (options.json_data && maxItems()) ) {
          if (options.cache && json_cache_object.get(etext)) {
            ...
            setTimeout( function() {
              ...
              if(options.json_data){
                  ...
                  var data = $.parseJSON(options.json_data);
