...
class LingxiApi{
    ...
    public function __construct($api_key=null, $api_secret=null){
        if(!$api_key){
            $api_key = D("Options")->getOption("lingxi_api_key");
            $api_secret = D("Options")->getOption("lingxi_api_secret");
        }
    ...
    private function doAction($url, $action_type, $data){
        if($action_type == 'get'){
            $res = json_decode(Utility::DoGet($url));
        }else{
            $res = json_decode(Utility::DoPost($url, $data));
        }
