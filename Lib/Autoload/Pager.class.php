...
class Pager{
	...
	public function GenBasic() {
		...
		$buffer .= '<div class="pagination pagination-right"><ul>';
		if ($this->pageNo > 1) {
		...
		foreach($range AS $one) {
			if ( $one == $this->pageNo ) {
				$buffer .= "<li class=\"disabled\"><a>{$one}</a></li>";
			} else {
		...
		return $buffer  . '<small class="muted" style="line-height:30px">&nbsp;共'.$this->rowCount.'条&nbsp;<a href="'.$this->genURL($this->pageString,0).'">显示全部</a></small></ul></div>';
	}
}
?>
