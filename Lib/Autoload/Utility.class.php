...
class Utility 
{
    ...
    static public function Radios($name, $a=array(), $v, $inline=false) {
        settype($a, 'array');
        $res = '';
        ...
        foreach($a AS $key=>$value) {
            ...
            $checked = $key==$v ? 'checked' : '';
            $res .= "<label class='radio {$inline_str}'><input type='radio' name='{$name}' value='{$key}' {$checked}> {$value}</label>";
        }
        return $res;
    }
    ...
    static function SmsContent($content) {
    ...
    static function GetCurrentWebRoot(){
        return "http://" . $_SERVER['HTTP_HOST'];
    }

    static function GetCurrentSubDomainName(){
        $domain = $_SERVER['HTTP_HOST'];
        $root_domain = D("Options")->getOption("root_domain");
        $subDomainName = str_replace($root_domain, "", $domain);
        $subDomainName = str_replace(".", "", $subDomainName);
